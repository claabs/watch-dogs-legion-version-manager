...
// https://gist.github.com/pgundlach/adf17ee49a64922a4a46
var cksumTbl = [256]uint32{0x00000000, 0x04C11DB7, 0x09823B6E, 0x0D4326D9,
...
func HashFileCRC32(filePath string, multiProgress *mpb.Progress) (uint32, error) {
	...
	// var tablePolynomial crc32.Table = cksumTbl
	// hash := crc32.New(&tablePolynomial)
	// if _, err := io.Copy(hash, file); err != nil {
	// 	return returnCRC32UInt, err
	// }
	// hashInBytes := hash.Sum(nil)[:]
	// returnCRC32UInt = binary.BigEndian.Uint32(hashInBytes)

	returnCRC32UInt, err = cksum(fileReader)
	// returnCRC32UInt = hash.Sum32()
	return returnCRC32UInt, err
}

// The standard CRC32 library isn't the same as POSIX cksum. This implementation is...
func cksum(rd io.Reader) (uint32, error) {
	...
	for {
		...
		crc = cksumTbl[byte(crc>>24)^v] ^ (crc << 8)
		length++
	}

	for extra := uint32(length); extra > 0; extra >>= 8 {
		crc = (crc << 8) ^ cksumTbl[byte(crc>>24)^byte(extra&0xff)]
	}
